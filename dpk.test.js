...
describe("deterministicPartitionKey", () => {
  ...
  it("Returns the same partitionKey when it doesn't exceed maximum partition key length", () => {
    ...
    const trivialKey = deterministicPartitionKey(validPartitionKeyObject);
    expect(trivialKey).toBe(validPartitionKeyObject.partitionKey);
  });
  ...
  it("Returns the same partitionKey as a string when the input is a number", () => {
    ...
    const trivialKey = deterministicPartitionKey(numericPartitionKeyObject);
  ...
  it("Returns a 128 length string when the input doesn't have a partitionKey property", () => {
    ...
    const trivialKey = deterministicPartitionKey(invalidObject);
  ...
  it("Returns a 128 length string when the input is longer than that", () => {
    ...
    const trivialKey = deterministicPartitionKey(longPartitionKeyObject);
    expect(trivialKey.length).toBe(128);
  });
});
